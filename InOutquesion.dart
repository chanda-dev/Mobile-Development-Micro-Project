...
class SingleChoice extends Question {
  ...
  SingleChoice(String title, List<String> options, this.correctAnswer, int score)
...
class MultipleChoice extends Question {
  ...
  MultipleChoice(String title, List<String> options, this.correctAnswers, int score)
      : super(title, options, score);

  @override
  bool isCorrect(List<String> answer) {
    return answer.toSet().containsAll(correctAnswers) && correctAnswers.toSet().containsAll(answer);
  }
}
...
class Quiz {
  ...
  void startQuiz() {
    for (var participant in participants) {
      ...
      for (var question in questions) {
        ...
        print('\nEnter your answer(s):');
        var answers = stdin.readLineSync()!.split(',').map((s) => s.trim()).toList();
  ...
  void displayResults() {
    print('\nQuiz Results:');
    for (var participant in participants) {
      print('${participant.getFullName()} scored: ${participant.totalScore}');
    }
  }
}
...
void main() {
  ...
  while (true) {
    ...
    List<String> options = stdin.readLineSync()!.split(',').map((s) => s.trim()).toList();
